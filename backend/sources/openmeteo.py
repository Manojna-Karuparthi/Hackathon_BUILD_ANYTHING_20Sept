...
class WeatherSource(Source):
    ...
    async def fetch(
        ...
        for zone, block in zip(zones, blocks):
            ...
            out[zone.id] = _summarise(precip, soil, hourly.get("time", []))
        self.store(key, out)
        return out
...
def _summarise(precip: list[float], soil: list[float], times: list[str]) -> dict[str, Any]:
    """Collapse an hourly series into the five numbers Channel A actually uses."""
